...
void Socket::connect(SocketAddress& _address, int _timeoutInSeconds){
    ...
    int errorCode = ::getaddrinfo(address.host.data(), std::to_string(address.port).data(), &hints, &results);
    
    if( errorCode) throw std::runtime_error("Failed to get address information: "+std::string(gai_strerror(errorCode)));
...
template<ByteReader T> int Socket::read(T& _reader){
...
void Socket::setSoTimeout(int _seconds){
    struct timeval tv {.tv_sec = _seconds };
